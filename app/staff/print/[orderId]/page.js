    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 flex flex-col h-full">
          <div>
            <div className="flex justify-between items-start border-b-2 pb-1 mb-2" style={{ borderColor: colorCode }}>
            ...
            <div className="flex gap-4 mb-2">
              <div className="flex-1 border p-1.5 bg-white/95 text-xs">
            ...
            <div className="border bg-white/95 mb-2">
              <table className="w-full text-xs text-left">
                <thead className="bg-gray-100 border-b">
                  <tr>
                    <th className="p-1.5">商品名・内容</th>
                    <th className="p-1.5 w-12 text-center">数量</th>
                    {showPrice && <th className="p-1.5 w-24 text-right">金額(税抜)</th>}
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b">
                    <td className="p-1.5">
                      ...
                      {o.cardType === '立札' && (
                        <div className="mt-1.5 p-1.5 border border-dashed border-gray-400 bg-gray-50 rounded w-fit pr-8">
                          <p className="text-[9px] text-gray-500 mb-0.5 font-bold">【立札の内容】</p>
                          <p className="font-bold text-[#c62828] text-[11px]">{o.tateInput1 || '祝'}</p>
                          <p className="font-bold text-[12px] text-gray-800">{o.tateInput3 || o.tateInput2}</p>
                        </div>
                      )}
                      
                      {o.cardType === 'メッセージカード' && (
                        <div className="mt-1.5 p-1.5 border border-dashed border-gray-400 bg-gray-50 rounded text-[10px] text-gray-800 italic">
                          「{o.cardMessage}」
                        </div>
                      )}
                    </td>
          ...
          {showSignature && (
            <div className="border border-gray-400 bg-white/95 p-2 mb-2 flex items-center justify-between">
              ...
              <div className="flex gap-4 items-end">
                <span className="text-[9px] text-gray-500">受領日：　　年　　月　　日</span>
                <div className="w-32 h-6 border-b border-dotted border-gray-400 flex items-end justify-end pb-0.5"><span className="text-[8px] text-gray-400">サインまたは印</span></div>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-between items-end pt-2 border-t border-gray-300">
            <div className="flex items-end gap-3">
              {generalConfig?.logoUrl && (
                <img src={generalConfig.logoUrl} alt="Logo" className="h-8 object-contain object-left mb-0.5" />
              )}
              <div className="text-[9px] text-gray-700 leading-tight">
                <p className="font-bold text-[13px] tracking-widest mb-1">{shopData?.name || 'FLORIX'}</p>
