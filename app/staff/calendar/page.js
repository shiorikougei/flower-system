...
export default function CalendarPage() {
  ...
  const handleSendEmail = (e) => {
    ...
    const d = selectedOrder?.order_data || {};
    if (!d?.customerInfo?.email) { alert("メールアドレスが登録されていません。"); return; }
    try {
  ...
  return (
    <main className="pb-32 font-sans text-left">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-[#111111]/60 backdrop-blur-sm p-3 md:p-4 animate-in fade-in text-left" onClick={() => setSelectedOrder(null)}>
            ...
            <div className="p-4 md:p-8 space-y-6 md:space-y-8 text-left overflow-x-hidden">
              <div className="bg-white p-5 rounded-[24px] border border-[#EAEAEA] shadow-sm flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <select 
                    value={modalData.currentStatus || 'new'} 
                    onChange={(e) => updateStatusValue(selectedOrder.id, e.target.value)}
                    className="h-10 bg-white border border-[#EAEAEA] rounded-xl px-3 text-[12px] font-bold outline-none cursor-pointer"
                  >
                ...
                <div className="bg-white p-6 rounded-[24px] border border-[#EAEAEA] shadow-sm space-y-4 text-left">
                  ...
                  {modalData.isRecipientDifferent ? (
                    <div className="space-y-1 text-left">
                      ...
                      <a href={getGoogleMapsUrl(modalTargetInfo)} target="_blank" className="inline-block mt-2 text-blue-500 text-[11px] font-bold hover:underline">Googleマップで見る</a>
                    </div>
              ...
              <div className="bg-white p-6 rounded-[24px] border border-[#EAEAEA] shadow-sm space-y-4 text-left">
                ...
                {modalData.cardType !== 'なし' && (
                  <div className="mt-4 p-4 bg-[#FBFAF9] rounded-xl border border-dashed border-[#CCC] space-y-2 text-left">
                    ...
                    <p className="text-[13px] font-bold whitespace-pre-wrap">{modalData.cardType === '立札' ? 
                      [modalData.tatePattern, modalData.tateInput1, modalData.tateInput2, modalData.tateInput3].filter(Boolean).join('\n') : 
              ...
              <div className="bg-white p-8 rounded-[32px] border-2 border-[#2D4B3E]/10 shadow-md space-y-4 text-left">
                ...
                <div className="space-y-2 text-[14px] font-bold text-[#555]">
                  <div className="flex justify-between"><span>商品代(税抜):</span><span>¥{Number(modalData.itemPrice).toLocaleString()}</span></div>
