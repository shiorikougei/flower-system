...
export default function SettingsPage() {
  ...
  const renderItemsTab = () => (
    <div className="space-y-8 animate-in fade-in">
        ...
        <div key={item.id} className="bg-white rounded-[32px] border p-8 shadow-sm relative space-y-6 text-left">
          ...
          <div className="pt-4 pb-2 border-t border-[#FBFAF9]">
            ...
            <div className="flex flex-wrap gap-3">
              <label className={`flex items-center gap-2 px-4 py-2 rounded-xl border cursor-pointer transition-all ${item.canPickup !== false ? 'bg-[#FBFAF9] border-[#2D4B3E]/30 text-[#2D4B3E]' : 'bg-white border-[#EAEAEA] text-[#999999]'}`}>
                <input type="checkbox" checked={item.canPickup !== false} onChange={(e)=>setFlowerItems(flowerItems.map(i=>i.id===item.id?{...i, canPickup:e.target.checked}:i))} className="accent-[#2D4B3E] w-4 h-4"/>
                <span className="text-[12px] font-bold flex items-center gap-1"><Store size={14}/> 店頭受取</span>
              </label>
              <label className={`flex items-center gap-2 px-4 py-2 rounded-xl border cursor-pointer transition-all ${item.canDelivery !== false ? 'bg-[#FBFAF9] border-[#2D4B3E]/30 text-[#2D4B3E]' : 'bg-white border-[#EAEAEA] text-[#999999]'}`}>
                <input type="checkbox" checked={item.canDelivery !== false} onChange={(e)=>setFlowerItems(flowerItems.map(i=>i.id===item.id?{...i, canDelivery:e.target.checked}:i))} className="accent-[#2D4B3E] w-4 h-4"/>
                <span className="text-[12px] font-bold flex items-center gap-1"><Truck size={14}/> 自社配達</span>
              </label>
              <label className={`flex items-center gap-2 px-4 py-2 rounded-xl border cursor-pointer transition-all ${item.canShipping !== false ? 'bg-[#FBFAF9] border-[#2D4B3E]/30 text-[#2D4B3E]' : 'bg-white border-[#EAEAEA] text-[#999999]'}`}>
                <input type="checkbox" checked={item.canShipping !== false} onChange={(e)=>setFlowerItems(flowerItems.map(i=>i.id===item.id?{...i, canShipping:e.target.checked}:i))} className="accent-[#2D4B3E] w-4 h-4"/>
                <span className="text-[12px] font-bold flex items-center gap-1"><Package size={14}/> 業者配送</span>
              </label>
            </div>
          </div>
      ...
      <button onClick={()=>setFlowerItems([...flowerItems, {id:Date.now(), name:'', normalLeadDays:2, shippingLeadDays:3, canBringFlowers:false, hasReturn:false, canPickup:true, canDelivery:true, canShipping:true}])} className="w-full py-10 border-2 border-dashed border-[#EAEAEA] rounded-[32px] text-[#999999] font-bold transition-all hover:border-[#2D4B3E]">+ 商品を追加</button>
    </div>
  );
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-orange-50 p-4 rounded-2xl border border-orange-200">
              <h3 className="text-[13px] font-bold text-orange-800 mb-3 flex items-center gap-1"><Store size={14}/> 店頭受取</h3>
            ...
            <div className="bg-blue-50 p-4 rounded-2xl border border-blue-200">
              <h3 className="text-[13px] font-bold text-blue-800 mb-3 flex items-center gap-1"><Truck size={14}/> 自社配達</h3>
              <div className="space-y-2">
            ...
            <div className="bg-green-50 p-4 rounded-2xl border border-green-200">
              <h3 className="text-[13px] font-bold text-green-800 mb-3 flex items-center gap-1"><Package size={14}/> 業者配送</h3>
              <div className="space-y-2">
