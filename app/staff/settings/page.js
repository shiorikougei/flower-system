...
import { 
  ...
  Trash2, Plus, Clock, ShieldCheck, RotateCcw, ImageIcon, Ruler, Percent, 
...
export default function SettingsPage() {
  ...
  // --- 3. 店舗管理 (詳細営業時間・配達時間・特別日) ---
  const [shops, setShops] = useState([]); 

  // --- 4. 商品管理 (納期・持込・返却) ---
  const [flowerItems, setFlowerItems] = useState([]);

  // --- 5. 配送・送料 (自社・業者・サイズ追加・地方追加・クール時期) ---
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('app_settings').upsert({ id: 'default', settings_data: payload });
  ...
  return (
    <div className="min-h-screen bg-[#FBFAF9] flex flex-col font-sans text-left pb-40">
      <header className="h-20 bg-white/80 backdrop-blur-md border-b flex items-center justify-between px-6 md:px-12 sticky top-0 z-50">
        <h1 className="text-[16px] font-bold text-[#2D4B3E] tracking-tight">FLORIX 設定</h1>
      ...
      <main className={`flex-1 max-w-[900px] mx-auto w-full py-10 px-6 transition-all ${!isAdmin ? 'opacity-40 pointer-events-none grayscale' : ''}`}>
        ...
        {activeTab === 'status' && (
          <div className="bg-white rounded-[32px] border p-8 shadow-sm space-y-6 animate-in fade-in">
            <h2 className="text-[18px] font-bold text-[#2D4B3E] flex items-center gap-2"><ListChecks size={20}/> ステータス設定</h2>
        ...
        {activeTab === 'shop' && (
          <div className="space-y-8 animate-in fade-in">
            {shops.map(shop => (
              <div key={shop.id} className="bg-white rounded-[32px] border p-8 shadow-sm relative space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1"><label className="text-[10px] font-bold text-[#999999]">郵便番号</label><input type="text" value={shop.zip} onChange={(e)=>setShops(shops.map(s=>s.id===shop.id?{...s, zip:e.target.value}:s))} className="w-full h-11 bg-[#FBFAF9] border rounded-xl px-4 text-[13px] font-bold outline-none"/></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t">
                  <div className="space-y-4">
                    <label className="text-[12px] font-bold text-[#2D4B3E] flex items-center gap-2"><Clock size={14}/> 店舗 営業時間・休業</label>
                  ...
                  <div className="space-y-4">
                    ...
                    {['canBringFlowers', 'canBringVase'].map(key => (
                      <div key={key} className="bg-[#FBFAF9] p-3 rounded-xl border space-y-2">
                        ...
                        {item[key] && <div className="flex items-center justify-between text-[10px]"><span>納期</span><div className="flex items-center gap-1"><input type="number" value={item[key+'LeadDays']||7} onChange={(e)=>setFlowerItems(flowerItems.map(i=>i.id===item.id?{...i, [key+'LeadDays']:Number(e.target.value)}:i))} className="w-10 border rounded text-center font-bold"/>日後</div></div>}
                      </div>
                    ))}
                  </div>
                  <div className="space-y-4">
                    ...
                    <div className="bg-[#FBFAF9] p-3 rounded-xl border space-y-3">
                      <label className="flex items-center justify-between text-[12px] font-bold cursor-pointer">器の回収/返却を必須にする<input type="checkbox" checked={item.hasReturn} onChange={(e)=>setFlowerItems(flowerItems.map(i=>i.id===item.id?{...i, hasReturn:e.target.checked}:i))} className="accent-[#2D4B3E] w-5 h-5"/></label>
                      <p className="text-[9px] text-[#999999] leading-tight font-bold">※オンにすると、注文フォームに器返却の確認と送料計算が追加されます。</p>
                    </div>
                  </div>
                </div>
              </div>
            ))}
            <button onClick={()=>setFlowerItems([...flowerItems, {id:Date.now(), name:'', normalLeadDays:2, shippingLeadDays:3, canBringFlowers:false, hasReturn:false}])} className="w-full py-10 border-2 border-dashed border-[#EAEAEA] rounded-[32px] text-[#999999] font-bold hover:border-[#2D4B3E] transition-all">+ 商品を登録</button>
          </div>
        )}
        ...
        {activeTab === 'shipping' && (
          <div className="space-y-8 animate-in fade-in text-left">
            <div className="bg-white rounded-[32px] border p-8 shadow-sm space-y-10">
              ...
              {/* 自社配達 */}
              <div className="space-y-4">
                <label className="text-[14px] font-bold text-[#2D4B3E] flex items-center gap-2">自社配達エリアと料金</label>
                <div className="grid grid-cols-1 gap-2">
                  {deliveryAreas.map(a => (
                    <div key={a.id} className="flex gap-2 bg-[#FBFAF9] p-2 rounded-xl border">
                      <input type="text" value={a.name} onChange={(e)=>setDeliveryAreas(deliveryAreas.map(x=>x.id===a.id?{...x, name:e.target.value}:x))} className="flex-[2] h-10 bg-white border rounded-xl px-3 text-xs font-bold" placeholder="エリア名（キーワード）"/>
                  ...
                  <button onClick={()=>setDeliveryAreas([...deliveryAreas, {id:Date.now(), name:'', fee:0}])} className="py-2 border border-dashed rounded-xl text-[11px] font-bold text-[#999999] hover:text-[#2D4B3E] transition-all">+ エリアを追加</button>
                </div>
              </div>

              {/* クール便期間 */}
              <div className="space-y-4 border-t pt-8">
                <div className="flex justify-between items-center"><label className="text-[14px] font-bold text-[#2D4B3E] flex items-center gap-2"><CalendarIcon size={16}/> クール便 加算適用期間</label><button onClick={()=>setBoxFeeConfig({...boxFeeConfig, coolBinPeriods: [...boxFeeConfig.coolBinPeriods, {id:Date.now(), start:'06-01', end:'09-30', note:''}]})} className="text-[10px] bg-[#2D4B3E] text-white px-3 py-1.5 rounded-full font-bold shadow-sm">+ 期間を追加</button></div>
                <div className="grid grid-cols-1 gap-3">
                  {boxFeeConfig.coolBinPeriods.map(p => (
                    <div key={p.id} className="flex flex-wrap gap-2 items-center bg-[#FBFAF9] p-3 rounded-2xl border border-[#EAEAEA]">
                      <div className="flex items-center gap-1 bg-white border rounded-lg px-2 py-1"><input type="text" value={p.start} onChange={(e)=>setBoxFeeConfig({...boxFeeConfig, coolBinPeriods: boxFeeConfig.coolBinPeriods.map(x=>x.id===p.id?{...x, start:e.target.value}:x)})} className="w-12 text-center text-[11px] font-bold outline-none"/></div>
                      <span className="text-[11px] text-[#999999]">〜</span>
                      <div className="flex items-center gap-1 bg-white border rounded-lg px-2 py-1"><input type="text" value={p.end} onChange={(e)=>setBoxFeeConfig({...boxFeeConfig, coolBinPeriods: boxFeeConfig.coolBinPeriods.map(x=>x.id===p.id?{...x, end:e.target.value}:x)})} className="w-12 text-center text-[11px] font-bold outline-none"/></div>
                      <input type="text" placeholder="理由（夏季など）" value={p.note} onChange={(e)=>setBoxFeeConfig({...boxFeeConfig, coolBinPeriods: boxFeeConfig.coolBinPeriods.map(x=>x.id===p.id?{...x, note:e.target.value}:x)})} className="flex-1 h-9 bg-white border rounded-lg px-3 text-[11px] font-bold outline-none"/>
              ...
              <div className="bg-[#2D4B3E]/5 p-6 rounded-[24px] border border-[#2D4B3E]/10 space-y-4">
                <div className="font-bold text-[#2D4B3E] text-[14px] flex items-center gap-2"><RotateCcw size={18}/> 器回収/返却時の送料加算</div>
              ...
              <div className="space-y-6 pt-4">
                <div className="flex justify-between items-center"><label className="text-[14px] font-bold text-[#2D4B3E] flex items-center gap-2"><Ruler size={16}/> 業者配送・サイズ別地方別送料マスタ</label>
                  <div className="flex gap-2">
                    ...
                    <button onClick={()=>{const r=prompt('地方名を入力(例:北陸)'); if(r) setShippingRates([...shippingRates, {region:r}]);}} className="text-[10px] bg-[#2D4B3E] text-white px-3 py-1.5 rounded-full font-bold shadow-sm">+ 地域追加</button>
                  </div>
                </div>
                <div className="overflow-x-auto border rounded-[24px]">
                  <table className="w-full text-left text-[10px] min-w-[1000px]">
                    <thead className="bg-[#FBFAF9] border-b text-[#999999]">
                      <tr>
                        <th className="p-3 w-32 font-bold">地方・地域名</th>
                        {shippingSizes.map(s=><th key={s} className="p-3 text-center border-l bg-gray-50">{s}サイズ</th>)}
                        {shippingSizes.map(s=><th key={'c'+s} className="p-3 text-center border-l bg-blue-50 text-blue-500">{s}クール</th>)}
                    ...
                    <tbody className="divide-y">
                      {shippingRates.map((r, i) => (
                        <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          {shippingSizes.map(s => <td key={'c'+s} className="p-1 border-l"><input type="number" value={r['cool'+s]||0} onChange={(e)=>{const n=[...shippingRates]; n[i]['cool'+s]=Number(e.target.value); setShippingRates(n);}} className="w-16 border rounded p-1.5 mx-auto block text-right text-blue-500 font-bold"/></td>)}
          ...
          <div className="bg-white rounded-[32px] border p-8 shadow-sm space-y-8 animate-in fade-in text-left">
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center justify-between bg-[#FBFAF9] p-5 rounded-[24px] cursor-pointer group hover:bg-white hover:shadow-md transition-all border border-transparent hover:border-[#EAEAEA]">
                <div><span className="font-bold block text-[14px] group-hover:text-[#2D4B3E]">お客様への自動返信メールを送らない</span><span className="text-[10px] text-[#999999] font-bold">代理入力時はメール通知を完全に停止します</span></div>
          ...
          <div className="bg-white rounded-[32px] border p-8 shadow-sm space-y-6 animate-in fade-in text-left">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col md:flex-row gap-2 pt-4 border-t">
                ...
                <select value={newStaffStore} onChange={(e)=>setNewStaffStore(e.target.value)} className="flex-1 h-12 bg-[#FBFAF9] border rounded-xl px-4 text-[12px] font-bold"><option value="all">全店舗共通</option>{shops.map(shop=><option key={shop.id} value={shop.id}>{shop.name}</option>)}</select>
        ...
        {activeTab === 'message' && (
          <div className="bg-white rounded-[32px] border p-8 shadow-sm space-y-6 animate-in fade-in text-left">
            ...
            <div className="space-y-4">
              <div className="space-y-1"><label className="text-[11px] font-bold text-[#999999]">メール件名</label><input type="text" value={autoReply.subject} onChange={(e)=>setAutoReply({...autoReply, subject:e.target.value})} className="w-full h-12 bg-[#FBFAF9] border rounded-xl px-4 font-bold outline-none focus:border-[#2D4B3E]"/></div>
              <div className="space-y-1"><label className="text-[11px] font-bold text-[#999999]">メール本文（{"{CustomerName}"} などのタグが使えます）</label><textarea value={autoReply.body} onChange={(e)=>setAutoReply({...autoReply, body:e.target.value})} className="w-full h-80 bg-[#FBFAF9] border rounded-[24px] p-5 text-[13px] font-bold outline-none resize-none leading-relaxed focus:border-[#2D4B3E]" /></div>
            </div>
          </div>
        )}
      </main>
