...
export default function SettingsPage() {
  ...
  const saveSettings = async () => {
    ...
    try {
      const settingsData = {
        generalConfig, shops, flowerItems, staffList, deliveryAreas, shippingProvider, shippingRates,
    ...
    <div className="min-h-screen bg-[#FBFAF9] flex flex-col md:flex-row text-[#111111] font-sans">
      <aside className="w-full md:w-64 bg-white border-r border-[#EAEAEA] md:fixed h-full z-20 overflow-y-auto pb-10">
        ...
        <nav className="p-4 space-y-1">
          ...
          <Link href="/staff/portfolio" className="block px-6 py-4 rounded-xl text-[#555555] hover:bg-[#F7F7F7] text-[13px] font-bold tracking-wider">作品・SNS連携</Link>
        </nav>
      ...
      <main className="flex-1 md:ml-64 flex flex-col min-w-0 text-left">
        ...
        <div className={`max-w-[840px] mx-auto w-full py-12 md:py-20 px-6 transition-all duration-500 ${!isAdmin ? 'pointer-events-none opacity-60 grayscale-[30%]' : ''}`}>
                  ...
                  <div className="flex flex-col lg:flex-row gap-8">
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="flex flex-col gap-4 p-6 bg-[#2D4B3E]/5 rounded-2xl border border-[#2D4B3E]/20">
                        ...
                        <p className="text-[10px] text-[#999999]">※右側のプレビューを見ながらスライダーで濃さを調整してください。</p>
                      </div>
                    </div>
                    <div className="w-full lg:w-[280px] shrink-0 space-y-2">
                       <label className="text-[11px] font-bold text-[#999999] tracking-widest text-center block">伝票プレビュー (受注書控)</label>
                       <div className="relative w-full aspect-[210/148] bg-[#f1f8e9] border border-gray-300 rounded-xl p-4 overflow-hidden flex flex-col justify-between shadow-md">
                         ...
                         ) : (<div className="absolute inset-0 flex items-center justify-center opacity-[0.03] pointer-events-none overflow-hidden z-0"><span className="font-serif text-[40px] font-black transform -rotate-12">{generalConfig.appName || 'FLORIX'}</span></div>)}
                         ...
                         <div className="relative z-10 flex gap-2 h-14 mt-2"><div className="flex-1 bg-white/90 border border-gray-300 p-1.5 text-[8px]"><span className="text-gray-500">【ご依頼主様】</span><br/><span className="text-[10px] font-bold">山田 太郎</span> 様</div><div className="flex-1 bg-white/90 border border-gray-300 p-1.5 text-[8px]"><span className="text-gray-500">【お届け先様】</span><br/><span className="text-[10px] font-bold">鈴木 花子</span> 様</div></div>
                         <div className="relative z-10 bg-white/90 border border-gray-300 mt-2 h-8 flex justify-end items-center p-2 text-[10px] font-bold">合計 ¥5,500</div>
                       </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="h-40 text-left"></div>
        </div>
      </main>
