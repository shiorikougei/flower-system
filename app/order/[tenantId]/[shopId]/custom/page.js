...
function OrderFormContent() {
  ...
  // [Phase2-⑦] 共通モジュール utils/tateMaster から取得（旧4ファイル重複定義を解消）
    ...
    if (method === 'delivery') {
      ...
      if (rateData) {
        ...
        if (bfc?.type === 'flat') {
          ...
          // boxFeeConfigなし: flowerItemのdefaultBoxSizeのみ
      ...
      if (matchedFee !== null) {
        ...
        if (appSettings?.boxFeeConfig?.applyToDelivery) {
          if (appSettings.boxFeeConfig.type === 'flat') {
            deliveryBoxFee = Number(appSettings.boxFeeConfig.flatFee) || 0;
          } else if (appSettings.boxFeeConfig.type === 'price_based') {
            const tiers = appSettings.boxFeeConfig.priceTiers || [];
            const sorted = [...tiers].sort((a, b) => b.minPrice - a.minPrice);
        ...
        setCalculatedFee(baseFee + deliveryBoxFee); setPickupFee(pickupFeeAmt); setFeeBreakdown({ baseFee, boxFee: deliveryBoxFee, coolFee: 0 }); setAreaError('');
      } else {
        ...
        {step === 4 && (
          <div className="space-y-10 animate-in fade-in duration-200">
            ...
            <div className="space-y-6">
              ...
              {/* ★ STEP 2.5 で選択済みの配達方法を確認表示（変更は STEP 2.5 へ戻る） */}
              {(receiveMethod === 'delivery' || receiveMethod === 'sagawa') && (
                (() => {
                  ...
                  return (
                    <>
                      {/* 配達方法の確認カード */}
                      <div className={`p-5 rounded-2xl border-2 ${isDelivery ? 'bg-emerald-50 border-emerald-200' : 'bg-blue-50 border-blue-200'}`}>
              ...
              <div className="bg-white p-6 rounded-2xl border border-[#EAEAEA] mt-6">
                ...
                {paymentMethod === 'bank_transfer' && (() => {
                  ...
                  if (paymentScheduledDate) {
                    ...
                    const wishDate = selectedDate ? new Date(selectedDate) : null;
                    const isLate = wishDate && start > wishDate; // 制作開始日が希望日より後＝間に合わない
                        ...
                        {scheduleInfo && scheduleInfo.isLate && (
                          <div className="bg-red-50 border-2 border-red-300 rounded-lg p-3 space-y-2">
                            ...
                            <label className="flex items-start gap-2 cursor-pointer pt-2 border-t border-red-200">
                              <input
                                type="checkbox"
                                checked={lateScheduleAgreed}
                                onChange={(e) => setLateScheduleAgreed(e.target.checked)}
